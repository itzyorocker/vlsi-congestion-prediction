"""
Evaluate the 64-patch-trained model on its validation patches.
For comparison against:
  - Full-image ResUNet (SIZE=256)  -> Pearson 0.1957
  - Patch-based (256 patches)      -> Pearson 0.1570
  - Patch-based (128 patches)      -> Pearson 0.2045
  - Patch-based (64 patches)       -> ?
"""
import torch
import torch.nn.functional as F
from pathlib import Path
from torch.utils.data import DataLoader, random_split
from train_patch64 import PatchCircuitNet, ResUNetSE, DEVICE

def nrms(p, t):
    rng = t.max() - t.min()
    if rng < 1e-4:               # near-empty patch -> skip, avoid divide-by-~0 blowup
        return None
...
def main():
    ...
    net.load_state_dict(torch.load(Path.home() / "vlsi-project/unet_patch64.pt",
    ...
    print(f"\nPATCH64 MODEL  ->  NRMS {avg_nrms:.4f}  (skipped {skipped}/{count} "
