"""
Evaluate the 7-channel, 128-patch model on its validation patches.
For comparison against the 3-channel 128-patch baseline:
  3 channels -> Pearson 0.2045 (10614 -> wait, that was 14592 patches, 3ch)
  7 channels -> ?  (10614 patches -- smaller matched subset, see notes)
"""
import torch
...
def main():
    ...
    net.load_state_dict(torch.load(Path.home() / "vlsi-project/unet_patch128_v2.pt",
    ...
    print(f"\nPATCH128_V2 (7ch) MODEL  ->  NRMS {avg_nrms:.4f}  "
          f"(skipped {skipped}/{count} near-empty)   "
