"""
Patch-based congestion predictor -- 64x64 patches.
Next data point in the patch-size sweep:
  256-patch -> Pearson 0.1570
  128-patch -> Pearson 0.2045  (best so far)
  64-patch  -> ?
Smaller patches use less VRAM, so batch size is bumped up.
"""
...
PATCH = 64               # native-resolution patch size, no resizing
BATCH_SIZE = 32          # smaller patches -> more VRAM headroom -> bigger batch
NUM_WORKERS = 4
EPOCHS = 30
LR = 2e-4
WEIGHT = 20.0
...
class PatchCircuitNet(Dataset):
    ...
    Tiles the full chip in a grid with this patch size (more, smaller tiles
    than the 128/256 versions).
    """
...
# ---- same GroupNorm Residual U-Net + SE as your working models ----
def gn(ch):
...
def main():
    ...
    best_val = float("inf")
    save_path = Path.home() / "vlsi-project/unet_patch64.pt"   # SEPARATE file
