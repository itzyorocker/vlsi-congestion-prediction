"""
Patch-based congestion predictor -- 128x128 patches, 7 channels, OVERLAPPING tiles.
Same setup as train_patch128_v2.py (which got Pearson 0.3107) but with
stride=64 instead of stride=128 -- patches now overlap by 50%, giving
roughly 4x more training samples from the same chips.
"""
...
PATCH = 128
STRIDE = 64              # was PATCH (128, non-overlapping) -- now 50% overlap
BATCH_SIZE = 16
NUM_WORKERS = 4
EPOCHS = 30
LR = 2e-4
...
class PatchCircuitNet(Dataset):
    def __init__(self, patch=PATCH, stride=STRIDE):
        self.patch = patch
        self.index = []
        for d in DESIGNS:
            base = DATA / d
            ...
            for f in sorted(tgt_dir.glob("*.npz")):
                ...
                # overlapping grid: step by `stride`, not `patch`
                ys = list(range(0, max(1, H - patch + 1), stride))
                xs = list(range(0, max(1, W - patch + 1), stride))
                if not ys: ys = [0]
                if not xs: xs = [0]
                # make sure the bottom/right edge is also covered
                if ys[-1] + patch < H: ys.append(max(0, H - patch))
                if xs[-1] + patch < W: xs.append(max(0, W - patch))
                for y0 in ys:
                    for x0 in xs:
                        self.index.append((paths, y0, x0))
        print(f"patch dataset size: {len(self.index)} patches "
              f"(patch={patch}x{patch}, stride={stride}, {len(IN_FEATS)} channels)")
...
def main():
    print("device:", DEVICE, "| PATCH:", PATCH, "| STRIDE:", STRIDE,
          "| channels:", len(IN_FEATS), "| batch:", BATCH_SIZE,
    ...
    best_val = float("inf")
    save_path = Path.home() / "vlsi-project/unet_patch128_v3.pt"   # SEPARATE file
