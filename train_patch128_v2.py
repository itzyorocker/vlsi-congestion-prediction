"""
Patch-based congestion predictor -- 128x128 patches, EXTRA CHANNELS.
Adds the previously-unused feature maps as extra input channels:
  RUDY_long, RUDY_short, RUDY_pin_long, cell_density
Baseline to beat: train_patch128.py -> Pearson 0.2045 (3 channels)
"""
...
# 7 input channels now, up from 3.
IN_FEATS = [
    "macro_region",
...
class PatchCircuitNet(Dataset):
    def __init__(self, patch=PATCH):
        ...
        for d in DESIGNS:
            ...
            for f in sorted(tgt_dir.glob("*.npz")):
                ...
                if not all(p.exists() for p in paths.values()):
                    continue          # skips any design missing a channel
        ...
        print(f"patch dataset size: {len(self.index)} patches "
              f"(patch={patch}x{patch}, {len(IN_FEATS)} input channels)")
...
class ResUNetSE(nn.Module):
    def __init__(self, cin=7, base=32):     # cin=7 now, was 3
        super().__init__()
...
def main():
    print("device:", DEVICE, "| PATCH:", PATCH, "| channels:", len(IN_FEATS),
          "| batch:", BATCH_SIZE, "| epochs:", EPOCHS, "| WEIGHT:", WEIGHT)
    ds = PatchCircuitNet()
    ...
    best_val = float("inf")
    save_path = Path.home() / "vlsi-project/unet_patch128_v2.pt"   # SEPARATE file
